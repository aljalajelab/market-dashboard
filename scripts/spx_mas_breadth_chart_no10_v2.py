# spx_mas_breadth_chart_no10_v2.py — يحفظ data/mas_21_50_200.csv
...
def download_prices(tickers,start,end):
    ...
    for i in range(0,len(tickers),60):
        ...
        for attempt in (1,2):
            try:
                data=yf.download(chunk,start=start.isoformat(),end=(end+dt.timedelta(days=1)).isoformat(),progress=False,auto_adjust=False,group_by="ticker",threads=True); break
            except:
                if attempt==2: raise
                time.sleep(2)
        parts.append(pd.DataFrame({t:data[(t,"Close")] for t in chunk if (t,"Close") in data}) if isinstance(data.columns,pd.MultiIndex) else pd.DataFrame({chunk[0]:data["Close"]}))
...
def main():
    ...
    for y in needed_years:
        ...
        idx=close.index[close.index.year==y]; cols=[t for t in year_to_tickers[y] if t in close.columns]
        if not len(idx) or not cols: continue
        cnd=(s21.loc[idx,cols]>s50.loc[idx,cols])&(s50.loc[idx,cols]>s200.loc[idx,cols])
        parts.append((cnd.sum(axis=1)/(~close.loc[idx,cols].isna()).sum(axis=1).replace(0,pd.NA))*100.0)
    breadth=pd.concat(parts).sort_index(); breadth.name="MAS_21_50_200"
