# qqqe_seasonality.py — يحفظ data/qqqe_seasonality.csv
import os, glob, re
import pandas as pd
import numpy as np
...
def pick_csv(path):
    preferred="QQQE ETF Stock Price History.csv"
    p=path/preferred
    if p.is_file(): return p
    cands=sorted(path.glob("*QQQE*.csv"),key=lambda x:x.stat().st_size,reverse=True)
    if cands: return cands[0]
    cands=sorted(path.glob("*.csv"),key=lambda x:x.stat().st_size,reverse=True)
    if cands: return cands[0]
    raise FileNotFoundError("QQQE CSV not found.")
...
def read_csv(path):
    ...
    close_col=None
    for k in ["Price","Close","Adj Close","Last"]:
        if k in cols: close_col=k; break
    if not close_col: close_col=df.columns[1]
...
pd.DataFrame({"tdoy":range(1,MAX_TDOY+1),"avg_ret":daily_avg.values,"cum_return_pct":cum_pct.values}).to_csv(DATA_DIR/"qqqe_seasonality.csv",index=False)
pd.DataFrame({"month":range(1,13),"avg_return_pct":[avg_month.get(m,np.nan)*100 for m in range(1,13)]}).to_csv(DATA_DIR/"qqqe_seasonality_monthly.csv",index=False)
print(f"✅ qqqe_seasonality.csv")
