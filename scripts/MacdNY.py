# MacdNY.py — يحفظ data/macd_ny.csv
from pathlib import Path
import numpy as np, pandas as pd
import matplotlib; matplotlib.use("Agg")
import yfinance as yf

START_DATE=pd.Timestamp("2017-01-01"); MIN_UNIVERSE=700; FAST_EMA=12; SLOW_EMA=26
BASE_DIR=Path(__file__).resolve().parent
DATA_DIR=BASE_DIR.parent/"data"; DATA_DIR.mkdir(exist_ok=True)

def find_tickers_file():
    for name in ["nyse.csv","nyse.txt","nyse_tickers.csv","tickers.csv"]:
        p=BASE_DIR/name
        if p.is_file(): return p
    for pat in ("*.csv","*.txt"):
        got=list(BASE_DIR.glob(pat))
        if got: return got[0]
    raise FileNotFoundError("ملف التيكرات غير موجود.")

def load_tickers(path):
    try:
        df=pd.read_csv(path,header=None,names=["ticker"]); s=df["ticker"]
    except:
        s=pd.Series(Path(path).read_text().splitlines(),name="ticker")
    return s.astype(str).str.strip().str.upper().replace("",np.nan).dropna().drop_duplicates().tolist()
...
def compute_macd_breadth(close_wide):
    ...
    return breadth[breadth.index>=START_DATE].dropna().rename("MACD_NY_Breadth")
...
def main():
    ...
    out.to_csv(DATA_DIR/"macd_ny.csv")
    print(f"✅ macd_ny.csv — آخر قيمة: {series.iloc[-1]:.2f}")
