# SpreadrsiNY.py — يحفظ data/rsi_spread_ny.csv
import time
...
START_DATE="2017-01-01"; RSI_PERIOD=14; BATCH_SIZE=60; PAUSE_SEC=0.8; MIN_UNIVERSE=700
...
def read_tickers(path):
    try:
        s=pd.read_csv(path,header=None,names=["ticker"])["ticker"]
    except:
        s=pd.Series(path.read_text(encoding="utf-8",errors="ignore").splitlines(),name="ticker")
...
def main():
    tickers=read_tickers(find_tickers_file()); frames=[]
